...
def crea_layout_GUI(root, lotti, aggiungi_lotto, modifica_lotto, elimina_lotto, on_treeview_click, mostra_grafico):
    """
    Crea il layout dell'interfaccia grafica.

    Args:
    root (Tk): La finestra principale.
    lotti (list): Lista dei lotti.
    aggiungi_lotto (function): Funzione per aggiungere un lotto.
    modifica_lotto (function): Funzione per modificare un lotto.
    elimina_lotto (function): Funzione per eliminare un lotto.
    on_treeview_click (function): Funzione da chiamare al click su un elemento della Treeview.
    mostra_grafico (function): Funzione per mostrare il grafico dei ratios.
    """
    frame = Frame(root)
    ...
    Button(frame, text="Mostra Grafico PE", command=lambda: mostra_grafico(entry_nome.get(), 'PE')).grid(row=4, column=0, padx=5, pady=5)
    Button(frame, text="Mostra Grafico PB", command=lambda: mostra_grafico(entry_nome.get(), 'PB')).grid(row=4, column=1, padx=5, pady=5)
...
def aggiorna_tabella(tree, lotti):
    """
    Aggiorna la tabella dei lotti.

    Args:
    tree (Treeview): La Treeview da aggiornare.
    lotti (list): Lista dei lotti da visualizzare.
    """
...
def on_treeview_click(event, entry_nome, entry_prezzo_acquisto, entry_quantita):
    """
    Gestisce il click su un elemento della Treeview.

    Args:
    event (Event): L'evento di click.
    entry_nome (Entry): Campo di input per il nome.
    entry_prezzo_acquisto (Entry): Campo di input per il prezzo di acquisto.
    entry_quantita (Entry): Campo di input per la quantità.
    """
...
def mostra_grafico(ticker, years, ratios, ratio_type):
    """
    Mostra un grafico per una serie storica di ratios.

    Args:
    ticker (str): Il simbolo del ticker.
    years (list): Lista degli anni.
    ratios (list): Lista dei valori dei ratios.
    ratio_type (str): Tipo di ratio ('PE' o 'PB').
